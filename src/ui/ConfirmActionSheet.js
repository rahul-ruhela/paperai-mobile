...
export default function ConfirmActionSheet({
  ...
  const handleCancel = () => {
    Animated.parallel([
      ...
      Animated.timing(sheet, { toValue: 0, duration: 140, useNativeDriver: true }),
    ]).start(() => onCancel?.());
  };
        ...
        <Animated.View style={[styles.sheet, { transform: [{ translateY }] }]}>
          ...
          <View style={styles.header}>
            <View style={[styles.iconWrap, destructive && styles.iconWrapDestructive]}>
              <Ionicons name={icon} size={20} color={destructive ? "#fff" : "#A5B4FC"} />
            </View>
            <View style={{ flex: 1 }}>
          ...
          <Pressable style={({ pressed }) => [styles.cancelBtn, pressed && { opacity: 0.85 }]} onPress={handleCancel}>
            <Text style={styles.cancelText}>{cancelText}</Text>
          </Pressable>

          <Text style={styles.legal}>
            {Platform.OS === "ios"
              ? "You can restore purchases anytime from the Paywall."
...
const styles = StyleSheet.create({
  ...
  message: { marginTop: 4, color: "rgba(255,255,255,0.72)", fontWeight: "600", lineHeight: 18 },
